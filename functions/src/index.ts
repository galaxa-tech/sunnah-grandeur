...
/**
 * 1. createStripeSession: HTTPS onRequest Cloud Function
 * Accepts userId, items (array with names, prices in cents, quantities), successUrl, and cancelUrl.
 * Securely constructs and returns a Stripe Checkout Session.
 */
...
/**
 * 2. stripeWebhook: HTTPS onRequest Cloud Function
 * Verifies Stripe signature using webhook secret, listens for checkout.session.completed,
 * extracts metadata, and writes the successful purchase directly to Cloud Firestore /orders collection.
 */
  ...
  async (req, res) => {
    ...
    try {
      if (webhookSecret && sig) {
        ...
        // Fallback for emulator testing if webhook secret is unconfigured
        event = req.body;
      }
    } catch (err: any) {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      try {
        ...
        // Required Order Schema: userId, items (array of items with price, quantity, productId), total (in cents), status ('paid'), createdAt (server timestamp)
        const orderData = {
          userId: userId,
